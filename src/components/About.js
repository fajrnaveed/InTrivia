import React from "react";
import LightSpeed from "react-reveal/LightSpeed";
import "../../src/index.css"
import fajr from "../assets/fajr.png";
import sameer from "../assets/sameer.png";
import zohaib from "../assets/zohaib.png";
import {  CDBFooterLink, CDBBtn, CDBIcon, CDBContainer, CDBBox, CDBFooter } from "cdbreact";
const about_img = {
  display: "flex-column",
  margin: "1rem 10rem",
  width: "13%",
};
const About = () => {
  return (
    <div>
      <div className=" d-flex justify-content-center p-5   ">
        <LightSpeed left>
          <h1>About Us</h1>
        </LightSpeed>
      </div>
      <div class="container-rounded p-1 my-2 bg-dark text-white border border-info  ">
        <img
          style={about_img}
          src={fajr}
          class="image rounded-circle  "
          alt=""
        />
        <img
          style={about_img}
          src={sameer}
          class="image rounded-circle "
          alt=""
        />
        <img
          style={about_img}
          src={zohaib}
          class="image rounded-circle "
          alt=""
        />

        <LightSpeed left>
          <h3>
            <div className="boxaboutusnames">
              <div className="mr-auto ml-auto">Fajr Naveed</div>
              <div className="mr-auto ml-auto">Sameer Khan</div>
              <div className="mr-auto ml-auto">Muhammad Zohaib</div>
            </div>
          </h3>
        </LightSpeed>
      </div>
      <div class="d-flex justify-content-center p-5 ml-5 mr-5">
        <h2>
          "We are students at Fast NUCES. We made this platform for people who
          don't have much experience in giving interviews. This will train them
          for the hardships they will be facing by giving multiple questions one
          after another and then telling them results of their session. They can
          give multiple interviews and analyse it which will help them for
          actual interview."
        </h2>
      </div>
      <CDBFooter className="shadow">
      <CDBBox
        display="flex"
        justifyContent="between"
        alignItems="center"
        className="mx-auto py-4 flex-wrap"
        style={{ width: '80%' }}
      >
        <CDBBox display="flex" alignItems="center">
          <a href="/" className="d-flex align-items-center p-0 text-dark">
            <img
              alt="logo"
              src="data:image/png;base64,"
              width="30px"
            />
            <span className="ml-4 h5 mb-0 font-weight-bold">Devwares</span>
          </a>
        </CDBBox>
        <CDBBox>
          <small className="ml-2">&copy; Devwares, 2022. All rights reserved.</small>
        </CDBBox>
        <CDBBox display="flex">
          <CDBBtn flat color="dark" className="p-2">
            <CDBIcon fab icon="facebook-f" />
          </CDBBtn>
          <CDBBtn flat color="dark" className="mx-3 p-2">
            <CDBIcon fab icon="twitter" />
          </CDBBtn>
          <CDBBtn flat color="dark" className="p-2">
            <CDBIcon fab icon="instagram" />
          </CDBBtn>
        </CDBBox>
      </CDBBox>
    </CDBFooter>

    </div>
  );
};

export default About;
